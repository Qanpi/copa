...
router.post("/teams", isAuthMiddleware, body("manager").isMongoId(), body("name").trim().isString().isLength({max: 30}).notEmpty().not().equals("[deleted]"), body("phoneNumber").isString().optional(), reportValidationErrors, teams.createOne);
